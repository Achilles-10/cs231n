...
def layernorm_backward(dout, cache):
    ...
    dx_norm = dout*gamma
#     print(x_norm.shape,ivar.shape,dx_norm.shape,dout.shape)
    dx_norm = dx_norm.T
    x_norm = x_norm.T
        ...
        for f in range(F):
            for h_out in range(H_out):
                for w_out in range(W_out):
                    out[n,f,h_out,w_out] = np.sum(x_pad[n, :, h_out*stride:h_out*stride+HH, w_out*stride:w_out*stride+WW]*w[f,:])+b[f]
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    cache = (x, w, b, conv_param)
...
def max_pool_backward_naive(dout, cache):
    ...
    for n in range(N):
        for c in range(C):
            for h_out in range(H_out):
                for w_out in range(W_out):
                    idx = np.argmax(x[n,c,h_out*stride:h_out*stride+pool_height,w_out*stride:w_out*stride+pool_width])
                    i,j=np.unravel_index(idx,(pool_height,pool_width))
                    dx[n,c,h_out*stride:h_out*stride+pool_height,w_out*stride:w_out*stride+pool_width][i,j]=dout[n,c,h_out,w_out]
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return dx
...
def spatial_groupnorm_backward(dout, cache):
    ...
    ###########################################################################
    N,C,H,W = dout.shape
    x_norm,gamma,x_mu,ivar,sqrt_var,var,eps,G = cache
    
    dbeta = np.sum(dout,axis=(0,2,3),keepdims=True)
    dgamma = np.sum(dout*x_norm,axis=(0,2,3),keepdims=True)
    
    dx_norm = dout*gamma[np.newaxis,:,np.newaxis,np.newaxis]
    dx_norm = dx_norm.reshape(-1,C//G*H*W).T
    x_norm = x_norm.reshape(-1,C//G*H*W).T
    N_norm,D_norm = dx_norm.shape
    dx = 1.0/N_norm * ivar * (N_norm*dx_norm - np.sum(dx_norm, axis=0) - x_norm*np.sum(dx_norm*x_norm, axis=0))
